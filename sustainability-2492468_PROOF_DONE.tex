...
\Title{\highlighting{Effects} of Water--Binder Ratio on Strength and Seismic Behavior of Stabilized Soil from Kongshavn, Port of Oslo}
%Dear authors,
...
\Author{\hl{Per Lindh}  $^{1,2}$\orcidA{} 
...
%2. Please confirm all ORCID number.
...
\address{%
$^{1}$ \quad Department of Investments Technology and Environment, Swedish Transport Administration, Neptunigatan~52, \hl{P.O. Box 366,} %MDPI: Please confirm if PO box can be remove as there are postcodes. 
 201 23~\highlighting{Malmö,} %MDPI: Please confirm if city name should be in English without accent.
 Sweden; \hl{p}%MDPI: We removed uppercase from email. please confirm.
er.lindh@trafikverket.se \hl{or} %MDPI: There are 2 emails for 1 author, please confirm if should  retained only one email.
 per.lindh@byggtek.lth.se  \\
 
$^{2}$ \quad Division of Building Materials, Department of Building and Environmental Technology, Lunds Tekniska Högskola LTH \hl{(Faculty of Engineering),} %MDPI: Please confirm to keep format of this information (in parentheses). same as in affiliation 3
 Lund University, \hl{P.O. Box 118,} \hl{221 00} %MDPI: we revised postcode  format same as affiliation 1. Please confirm.
 Lund, Sweden\\
 
$^{3}$ \quad Laboratory of Image Synthesis and Analysis (LISA), École Polytechnique de Bruxelles \hl{(Brussels Faculty of Engineering),} Université Libre de Bruxelles (ULB), \hl{Building L, Campus de Solbosch, ULB---LISA CP165/57,} %MDPI: %MDPI: Please check that the address information is complete. The provided information should be arranged from subordinate to superior.
 Avenue Franklin Roosevelt 50, \hl{1000} %MDPI: We moved postcode before city.
 Brussels, Belgium
}
...
\abstract{In many civil engineering problems, soil is stabilized by a combination of binders and {water. The} success of stabilization {is} evaluated {using} seismic {tests} with measured P-wave velocities. Optimization of process, laboratory testing and data modelling {are} essential {to} reduce the costs of the industrial projects. {This} paper reports the optimized workflow of soil stabilization through evaluated effects from the two factors controlling the development of strength{:} (1) the ratio between water and binder{;} (2) the proportions of different binders (cement/slag) were changed experimentally in a mixture of samples to evaluate the strength of soil. The experimental results show an optimal combination {of} 30\% cement {and} 70\% slag with a binder content of 120 \highlighting{kg/m$^3$} %MDPI: We removed the italics from all units. Please confirm this revision whole manuscript.
 and a maximum water binder ratio \highlighting{(w/b)} %MDPI: Please confirm if should be italic as equation. Please confirm whole manuscript.
 of 5. Such proportions of mixture demonstrated effective soil stabilization both on a pilot test scale and on full scale for industrial works. The correlation between {the} compressive strength and relative deformation of specimens {revealed} that {strength has} the highest values for \mbox{\highlighting{w/b} = 5} and the lowest for \mbox{\highlighting{w/b} = 7}. In {case} of high water content in soil and wet samples,\linebreak  the condition of a \highlighting{w/b} $\le$ 5 will require a higher amount of binder.}

\keyword{compressive strength; cement; slag; P-wave velocity; elastic body waves; seismic waves; curing; deformation; materials science; \highlighting{engineering} %MDPI: We removed PACS,MSC,JEL information following requirement of journal. Please confirm this revision.
} 
...
\end{adjustwidth}
\caption{\hl{Map} %MDPI: Please provide explanation for yellow triangle.
...
\caption{\hl{Status} %MDPI: We made alignment into center, please confirm.
...
\bf{\hl{Norway} %MDPI: Please confirm if the bold should be retained.
} &  &  &  &  \\
\midrule
Bærum & \hl{X} %MDPI: Please provide explanation for X.
 & \hl{--} %MDPI: Please confirm if en dash can be hyphen for `none of information'.
 & -- & -- \\
Gilhus & -- & -- & -- & X\\
Grenland	& -- & -- & -- & X\\
Bergen & -- & -- & -- & X\\
...
Soil samples {evaluated in this study }were collected as dredged sediments from {the }Port of Oslo, Norway. The tests points are located in {the }Kongshavn harbour and marked No. \hl{1\_2,} %MDPI: Please confirm if this should be 1, 2 and use en dash for  number 1-6. Please confirm whole manuscript.
...
%\end{adjustwidth}
\caption{\hl{Density} %MDPI: 1. Pelase confirm if the asterisk indicates multiply sign in ''10v*10c'', if not add explanation for it. 2. We moved figure 3 to here for reduced empty space. Please confirm.
...
%\end{adjustwidth}
\caption{Grain distribution in the different samples. Samples \hl{1\_2,} %MDPI: Please confirm if this should be 1,2 and use en dash for range number 1-4. Please confirm whole manuscript.
...
%\end{adjustwidth}
\caption{\hl{Grain} %MDPI: 1. Please provide explanation for black line with black cross sign. 2. The 2nd explanation in figure (Red line+black plus sign), please confirm if should be add the line in picture, we are not able to find it
...
The tests were implemented using various concentrations of water in a mixture of sediments collected from the Port of Oslo with respect to the amount of binder and\linebreak  water/binder ratio (\hl{w/b}). The selection of the most optimal \hl{w/b} value was argued based on the previous research performed in Arendal 2 and Gothenburg (southern Sweden) \cite{Lemenkova202139}. During the {soil-}testing procedure, the experiments were carried out with w/b of \hl{5--7} %MDPI: We used endash for range number, please confirm.
 of the dredged materials, where the lowest level corresponds to the highest amount of binder in relation to the amount of water in the dredged materials, i.e., ``high binder--low water'' $H_BL_W$. Accordingly, the highest level of \hl{w/b} {ratio }indicates the highest percentage of water and the lowest amount of binder. The \hl{w/b} ratio in fabricated specimens is reported in Table~\ref{tab02}. Here, the data include density of soil samples, and water/binder amounts indicating the percentage of {the }added binder to the mixture. 
\begin{figure}[H]
...
{Table} \ref{tab03} {summarizes the data on the measured samples and the results of the compressive strength (UCS, kPa) using mechanical tests with varied \hl{w/b} ratios. More specifically, Table} \ref{tab03} shows the values of the compressive strength {according to the different} \hl{w/b} {ratios }for specimens collected in various test sites (TS). {The measurements of the UCS correspond to the columns 3, 6 and 9 in Table} \ref{tab03}). {The} UCS tests were carried out according to the existing Swedish standard SS-EN 16907-4 \cite{SIS2018} with recommended deformation rate \mbox{of 1 mm/min} for soil specimens with a height of 100 mm. The results are summarized in Table \ref{tab03}. For the point $1\_2$, the \hl{w/b} = \hl{5--7} were tested for each specimen. As the ratio between water and binder controls the strength, samples with \hl{w/b} = 5 had higher strength compared to {the }samples with \hl{w/b} = 6 or 7, which proved the expected results. 

The specimens within the same \hl{w/b}, for instance, ID No. $1\_1$ to $1\_4$, show smaller variation in strength values. The same applies to the other \hl{w/b} ratios. Test samples No.~1\_1 and test body $1\_2$, however, show a different stiffness compared to other samples. Such{ }variation in stiffness is normal and is explained by{ }natural individual parameters of {the }soil masses. The results of the tests on the compressive strength and water/binder ratio for specimens collected from the sampling point No. 3 are also shown in the same {dataset, }Table \ref{tab03}. Specimen $4\_4$ from TS 4 obtained a significantly higher strength compared to{ }others with the same \hl{w/b} and should therefore be considered as an outlier.
\begin{table}[H] 
\caption{\hl{Compressive} %MDPI: We removed vertical line. Please confirm.
 strength (kPa) measured by UCS tests with relation to the water/binder ratio (\hl{w/b}) for specimens collected in sample test sites (TS) No. \hl{1\_2, 3 and 4.} %MDPI: Please confirm if should use en dash for range numbers, 1-4.
\label{tab03}}
\begin{tabularx}{\textwidth}{CCCCCCCCC}
\toprule
\textbf{TS 1\_2} & \textbf{\hl{w/b}} & \textbf{UCS} & \textbf{TS 3} & \textbf{\hl{w/b}} & \textbf{UCS} & \textbf{TS 4} & \textbf{\hl{w/b}} & \textbf{UCS}\\
\midrule
...
\hl{--} %MDPI: Please confirm if should be normal hyphen.
 & -- & -- & $3\_14$ & 7 & 150 & -- & -- & -- \\
\bottomrule
\end{tabularx}
\end{table}
...
Table \ref{tab04} shows the compressive strength with relation to the effects of changed water/binder ratio (\hl{w/b}) for the test points No. 5, 6 and 8, and additionally, the P-wave velocity for test site 5. {Here, soil stabilized by the blended cement-slag mixtures and water added in various proportions into the mixture indicates good performance in stabilization. This is achieved due to balanced \hl{w/b} proportions. This combination outperforms that of soil treated by cement-only binders. }With regard to the obtained results of the UCS tests, P-wave measurements were performed to analyze {the }correlation of the P-wave velocity {against the} compressive strength in {evaluated }soil samples. Therefore, after testing {the }P-wave velocity, the values of the compressive strength were calculated based on the recorded data on the speed of waves, which correlate with the strength of the materials. These results are reported in Table \ref{tab04}. 
\begin{table}[H] 
\caption{\hl{Compressive} %MDPI: We removed vertical line. Please confirm.
 strength (kPa) measured by UCS tests with regard to water/binder ratio (\hl{w/b}) for samples collected in test sites (TS) No. 5, 6 and 8, and P-wave velocity (m/s) for TS 5.\label{tab04}}
\begin{tabularx}{\textwidth}{CCCCCCCCCC}
\toprule
\textbf{TS 5} & \textbf{\hl{w/b}} & \textbf{\hl{P-w} %MDPI: Please confirm if should be ``P-wave''.
} & \textbf{UCS} & \textbf{TS 6} & \textbf{\hl{w/b}} & \textbf{UCS} & \textbf{TS 8} & \textbf{\hl{w/b}} & \textbf{UCS}\\
\midrule
...
Figure \ref{fig10} shows a graph of correlation between the pressure (kPa) and relative\linebreak  deformation of the {soil specimens collected} from {the }sample tests site point \hl{No.} %MDPI: We removed the italics. Please confirm this revision.
...
\caption{Correlation between compression (kPa) and relative deformation of 10 specimens collected from the sample test site No. \hl{1\_2.} \label{fig10}}
\end{figure}

The increase in relative deformation continues rapidly until {the values of the }compression strength {reach} 800 kPa, after which they stabilize and then decrease. Possible distortions and noise on the samples 11 to 13 are identified and classified as unstable responses to {the content of }the binder by {the }selected {soil }samples, since these {specimens }were compressed with relatively low strength {that} does not exceed 300 kPa. Samples 1 to 8 demonstrate {a} stable dynamics in measurements. In all cases, the relative deformation was measured until a value of 2.2\%. {The comparison of the plots for soil samples collected in test sites (TS) from 1 to 13 suggests the direct effects from the water/binder ratio on the hydration of cement that leads to the gain in strength (UCS). Such findings are necessary to account for the identification of the best combinations of binders and \hl{w/b} ratio, while optimizing binder blends and selecting the proportions of water in recipes.} %MDPI: some changes made in this paragraph

Figure \ref{fig11}{ shows} a graph of {the }pressure-deformation relationship for {soil }samples collected from the test point No. 3 with three different levels of \hl{w/b}, {which }are clearly visible in the graph. Randomly coloured line traces indicate selected sample specimens tested for {evaluation of soil }deformation{. }The first number in the numbering is the test point, in this case point No. 1\_2. The second number is a serial number. Samples 1 to 4 have \hl{w/b} = 5, samples 6 to 8 have \hl{w/b} = 6 and samples 11 to 13 have \hl{w/b} = 7. {The }variations in strength prove the expected results with the highest values for \hl{w/b} = 5 and the lowest strength for \hl{w/b} = 7{, respectively}. 
...
Accordingly, for {the} sample point No. 3, the expected pattern is shown in the same way in {soil }specimens with values of water/binder ratio between 5--7, see Figure \ref{fig11}. Here, the fragments of the modelled lines were annotated and randomly coloured using {the }IDs of soil samples and records on deformation value against {the }compression (kPa). However, the specimens collected from the sample test point $3\_1$ exhibit {a }slightly lower {level of }strength compared to other two test samples with \hl{w/b} 5. For samples from the point No. 3, less variation in stiffness was detected compared to the samples collected from the test point No. $1\_2$. {Here,} we also demonstrated the relationship between the compressive strength and {a }relative deformation, which reaches peak at 850 kPa, after which {it }stabilizes and {then }decreases. 
...
{The parabola function well represents the relationship between P-wave velocity and compressive strength in Figure} \ref{fig12}, {which clearly depicts its nonlinear nature. The correlation of P-wave-velocity and compressive strength well differs from the curves presented in earlier plots and has a steeper curvature, since it represents the relationship of the speed of the elastic waves on the stiffness  or porosity of the solid material. The tests were performed on soil samples, which have varying \hl{w/b} ratios in the binder mixture cement/slag content. Since the behaviour of elastic waves released by the accelerator and processed back by the receiver differs for samples stabilized with blended mixtures, various values for dots in Figure} \ref{fig12} {well illustrates the effects and minor variations of \hl{w/b} mixtures in slag-cement binders on soil strength. Nevertheless, the trend curve shows a general direction of the relationship between P-waves and strength in soil as a parabola function. Soil samples stabilized with cement/slag binders and different \hl{w/b} ratios can be distinguished as individual dots on a graph.}
\begin{figure}[H]
%\centering
	\includegraphics[width=13.0 cm]{Fig_12.jpg}
\caption{\hl{P-wave} %MDPI: Please check if an explanation should be added for dot line, and blue dots.
 velocity as a function of the compressive strength (UCS). \label{fig12}}
\end{figure}
...
Tests with iteratively changed binder parameters were used to select the best combinations by utilizing various \hl{w/b} ratios in a recipe of blended binders. The differences in P-wave behavior and UCS indirectly indicate variations in the porosity of soil stabilized with various blended mixes (cement/slag) and \hl{w/b} ratios, and these are compared with the soil samples recently stabilized, as shown in Figure \ref{fig12}. %MDPI: changes made in this paragraph
...
\caption{\hl{P-wave} %MDPI: Please revise color for the lines of Regression A and Regression Oslo, because thy're both in black and it's hard to distinguish 
 velocity as a function of the compressive strength \hl{(UCS).} %MDPI: the caption of figures 12 and 13 are the same, please check and revise
 \label{fig13}}
\end{figure}

{In Figure} \ref{fig13}, the P-wave velocity has been evaluated from samples collected from Arendal (A), V\hl{"}%MDPI: please confirm if the '' should be deleted
aster\aa s (V), Oslo (O), and Timr\aa~(two sites T1 and T2) after the curing period. The analysis of the UCS data interprets the degree of cement hydration, indicating the soil stabilization process from the speed of the P-waves that correlate with increased soil strength during stabilization.} The maximal level of {soil }strength is achieved at the \hl{w/b} \mbox{ratio of 5} where P-waves reach the recorded velocity of 1161 m/s. Such a good performance has a theoretical explanation of {the }optimized plasticity and viscosity of the binder recipe, which improves the mechanical properties of {the }cured soil. In particular, we derive the development of P-wave velocity with curing time{ }up to 60 days to demonstrate the effects from {the }two factors: the dynamics of strength over time and the influence of water--binder percentage on soil setting. 

{This part of the study was conducted as a critical test case to comparatively evaluate the behavior of the treated soil samples relative to the locations and environmental conditions of the original raw materials. The regression analysis demonstrates the statistical processing of the data collected in Oslo (represented by black crosses) at the end of the curing period for different combinations of w/b ratios. We visually analyzed the similarity between the curves for these soil samples, which were tested with varied \hl{w/b} proportions and collected from different locations. The goal was to compare the effects of increasing the ratio of stabilizing agents while decreasing water content.

The graph illustrates a clear visual similarity in curves, with soil collected from the Timr\aa~site exhibiting steeper curves compared to the other locations. This indicates soil samples with the highest percentage of binders relative to water content. } %MDPI: changes made in this paragraph
...
\midrule
\hl{0.0383} %MDPI: We used decimal point instead of dot. Please confirm.
 & 0.0298 & 0.00552 & <0.0005 & 0.0379 & <0.0002 & 0.814 & <0.02 & 6.90 & 0.00430 & 0.00831 & <0.03 \\
\midrule
\textbf{\hl{Sum PCB-7} %MDPI: Please confirm if the bold should be retained.
} & \textbf{\hl{Ca}} & \textbf{\hl{Fe}} & \textbf{\hl{K}} & \textbf{\hl{Mg}} & \textbf{\hl{Na}} & \textbf{\hl{Al}} & \textbf{\hl{B}} & \textbf{\hl{Ba}} & \textbf{\hl{Co}} & \textbf{\hl{V}} & \textbf{\hl{TBT}} \\
\midrule
...
\midrule
B1s2 & <\hl{0.00007} %MDPI: We used decimal point instead of dot. Please confirm.
...
\begin{itemize}
  \item We proved that dependency of strength of clayey soil on \hl{w/b} ratio can be modelled by a linear combination of tested specimens showing the gain of strength, and the improved ecological properties of soil---leaching of heavy metals and toxic substances. 
  \item The assumptions on the effects from {the }changed \hl{w/b} dosage on{ the }behaviour {of the marine sediments }is tested by adding various percentages of admixtures of cement/slag as binders.
  ...
  \item We showed that the combination of {the }P-waves and UCS methods for evaluating soil properties outperforms the standard state-of-the-art approach in two ways. 
  	\begin{itemize}[leftmargin=0.8cm,labelsep=4.9mm]
		...
		\item Second, the analyzed sensitivity of the ICP Accelerometer for measured resonance frequencies of sediments proved its high perception to varied {levels of }stiffness and viscosity of {soil }samples stabilized with different \hl{w/b} dosages.
	\end{itemize}
\end{itemize}

The essential practical advantage of the presented work consists in the optimization of {the }soil stabilization process in marine harbours {prior to construction works where large quantities of soil are to be stabilized. For instance, optimization of the processing of }millions of tons of the dredged sediment material {using} s/s techniques {increases cost-effectiveness}. Thus, the optimization of s/s techniques in such large-scale projects is crucial. The optimization of {the} \hl{w/b} dosage supports finding the best analytical solution for cost-effective s/s treatment of soil from marine harbours, when sediments are collected regularly and in large quantities. Using our data, the information on soil strength depending on {the} \hl{w/b} ratio and deformation of clayey soil specimens in the process of compression can be generalized in large-scale projects. {Overall, the results of seismic and UCS measurements demonstrated in this study show a similar trend and agreement in the estimated strength of stabilized specimens. The strength increased over the curing time in stabilized soil samples, as indicated by both seismic and UCS measurements.} %MDPI: some changes made in this last paragraph
\vspace{6pt}
...
\funding{\hl{The publication was funded by the Editorial Office of} %MDPI: discount infomation from the Editorial Office is not allowed to be put in the manuscript, please consider removing this part and change it to ''This research received no external funding''
 \emph{Sustainability}, Multidisciplinary Digital Publishing Institute (MDPI), by providing 100\% discount for the APC of this manuscript. }
...
\acknowledgments{The authors thank the reviewers for reading, suggestions and comments that improved an earlier version of this manuscript. \hl{The authors express sincere gratitude to the Editorial Office of Sustainability, MDPI for financial support of this publication.} %MDPI: please consider removing this part. Thank you very much
}
...
\reftitle{\highlighting{References} %MDPI: Please do NOT change the reference format with EndNote and other tools. Our production editor has done thorough layout work for reference. If any changes are necessary, please use the “Track changes function”. Thanks for your cooperation.
}
...
\newblock In Proceedings of the GeoCongress 2008, \hl{New Orleans, LA, USA, 9--12 March 2008;} %MDPI: Newly added information. Please confirm.
  ASCE: \hl{Reston, VA, USA,}  2008; pp. 772--779.
...
Dermatas, D.; Dutko, P.; Balorda-Barone, J.; Moon, D.H. Geotechnical
  ...
  Fill.
\newblock In {\em Dredging '02}; ASCE: \hl{Reston, VA, USA,} 2002; pp. 1--14.
...
\newblock In {\em Grouting and Deep Mixing 2012}; ASCE: \hl{Reston, VA, USA,}  2012; pp. 1800--1807.
...
\newblock In Proceedings of the Geo-Congress 2019, \hl{Philadelphia, PA, USA, 24--27 March} 2019; ASCE: \hl{Reston, VA, USA,}  2019; pp. 628--639.
...
\newblock In Proceedings of the ICCTP 2011, \hl{Nanjing, China, 14--17 August} 2011; ASCE: \hl{Reston, VA, USA,}  2011; pp. 3701--3705.
...
Taj, T.H.; Rahman, F.; Islam, M.R.; Rahman, M.A.; Hossain, M.I.; Hossain, S.A.;
  ...
  Mixing Piles.
\newblock In Proceedings of the International Conference on Transportation and Development, \hl{Seattle, WA, USA, 31 May--3 June} 2022; ASCE: \hl{Reston, VA, USA,}  2022; pp. 133--142.
...
\newblock In {\em Characterization of Minerals, Metals, and Materials 2016};
  John Wiley \& Sons, Ltd.:  \hl{Hoboken, NJ, USA,} 2016; Chapter~31, pp. 255--261.
...
\newblock In {\em Ground Improvement and Geosynthetics}; ASCE: \hl{Reston, VA, USA,} 2014; pp.
  148--157.
...
\newblock In {\em Geotechnical Frontiers 2017}; ASCE: \hl{Reston, VA, USA,} 2017; pp. 891--899.
...
Shrestha, R.; Al-Tabbaa, A. Development of Predictive Models for Cement
  Stabilized Soils.
\newblock In {\em Grouting and Deep Mixing 2012}; ASCE: \hl{Reston, VA, USA,} 2012; pp. 221--230.
...
Yesiller, N.; Hanson, J.L.; Usmen, M.A. Ultrasonic Assessment of Stabilized
  Soils.
\newblock In {\em Soft Ground Technology}; ASCE: \hl{Reston, VA, USA,}  2012; pp. 170--181.
...
\newblock {\em Geotech. Geol. Eng.} {\bf 2018}, \hl{1--10.} %MDPI: Please provide volume number.
...
\newblock In Proceedings of the Geo-Chicago, \hl{Chicago, IL, USA, 14--18 August} 2016; ASCE: \hl{Reston, VA, USA,} 2016; pp. 567--576.
...
\newblock In \emph{Recent Advances in Civil Engineering};  
 Kumar, P.G., Subramaniam, K.V.L., Santhakumar, S.M., Satyam~D.N., Eds.; \hl{Springer:} %MDPI: Newly added  information. Please confirm.
 Singapore,
   2022; pp. \hl{317--329.} %MDPI: We removed extra information ``IOP conference series: Materials science and engineering, vol 1006, No 1, IOP Publishing, pp 012020, 2020''. Please confirm if should be make to new ref. item and reorder of reference items into numerical order.
...
\hl{Zeffer, A.} %MDPI: We revised the author name, please confirm
 \emph{Sedimentprovtagning i Småbåtshamnar i Stenungsund [Sediment
  Sampling in Marinas in Stenungsund]};  
\newblock \hl{Stenungsunds Kommun,} %MDPI: We made ref into book format. Please add the name of the publisher before city or please add the URL link for the ref
 Sweden, 2011;
...
\newblock Technical Report 1-1009-0647; Stabilization and Solidification of Contaminated Dredge Masses. SGI  Information 20; %MDPI: We moved information before publisher. Please confirm.
 Swedish Geotechnical Institute (SGI):  Linköping, Sweden,  2011. 

\bibitem[{Swedish Institute for Standards (SIS)}(2011)]{SIS2011}
\emph{\hl{SS-EN} %MDPI: We formatted it as standard format. Please confirm and same below.
 197-1:2011};
...
\newblock
  Available online: \url{https://www.sis.se/en/produkter/construction-materials-and-building/construction-materials/cement-gypsum-lime-mortar/ssen197120112/} \hl{(accessed on).} %MDPI: Please add the access date (Format: Date Month Year). e.g., (accessed on 1 January 2020).
...
\newblock {Ground Granulated Blast Furnace Slag for Use in Concrete, Mortar and
  Grout---Part 1: Definitions, Specifications and Conformity Criteria}. Swedish Institute for Standards (SIS): {Stockholm, Sweden,} 2006.      Available online: \url{https://www.sis.se/en/produkter/standardization/vocabularies/construction-materials-and-building-vocabularies/ssen1516712006/} \hl{(accessed on}).
...
Irfan, M.; Uchimura, T. Measuring Shear and Compression Wave Velocities in
  ...
  Transducer.
\newblock In Proceedings of the  IACGE 2013, \hl{Chengdu, China, 25--27 October} 2013;   IACGE: \hl{Irvine, CA, USA,} 2013; pp. 414--421.
...
\newblock {\em {\hl{Dyn. Geotech. Test. ASTM STP} %MDPI: Please reconfirm journal name because different from information we found.
}} {\bf 1978}, {\em
  654},~\hl{91--125.} %MDPI: We removed publisher name from journal type. Please confirm.
...
\newblock {\em Materials} {\bf 2022}, {\em 15},~\hl{7798}.
...
\newblock
  Available online: \url{https://www.sis.se/en/produkter/environment-health-protection-safety/wastes/solid-wastes/ssen124572/} (\hl{accessed on}).
...
\newblock In Proceedings of the 4th International Conference on Innovative Materials, Structures
  and Technologies (IMST 2019), Riga, Latvia, \hl{25--27 September} 2019; Volume 660, p. 012071.
...
\newblock In Proceedings of the GeoCongress 2008, \hl{New Orleans, LA, USA, 9--12 March 2008;} ASCE: \hl{Reston, VA, USA,}  2008; pp. 668--675.
