...
The {mechanical} behaviour of low performing soils such as dredged sediments can be strengthened with binders. To this end, solidification/stabilization (s/s) {is used }to increase bearing capacity of soil through {the }improved strength, and the results are \mbox{encouraging~\cite{McLeod,Yongfeng,Rashidi,Zhangetal2011}.} A popular approach to s/s soil processing is to use binders as stabilizing agents, and then apply geotechnical methods of evaluating {the soil's} %MDPI: added "the soil's" for clarity; please ensure meaning has been retained. %Authors' reply: yes, correct, the meaning has been retained.
...
{The effects of varying water--binder ratios on the strength of stabilized soil, which rarely occur in practice, make it challenging to conduct tractable investigations and compare how changes in percentage directly affect the material's properties. %MDPI: changes made in this sentence; please check. %Authors' reply: yes, agree.
...
\end{adjustwidth}
\caption{Map %MDPI: Please provide explanation for yellow triangle. % <-- Authors' reply: yellow triangle is "Soils and sediments". The map is replotted and corrected.
...
\toprule
\textbf{ID} & \textbf{w %MDPI: Please confirm if should be italic as equation. % <-- Authors' reply: no, it is a sign for water ration (explained below the table).
} & \boldmath{$\gamma$} & \boldmath{$m_t$} & \boldmath{$m_s$} & \boldmath{$m_w$} & \textbf{w/b %MDPI: Please confirm if should be italic as equation. Please confirm whole manuscript. % <-- Authors' reply: no, this is an abbreviation "water/binder" (explained above and checked the whole manuscript).
} & \boldmath{$m_b$} & \boldmath{$m_{cement}$} & \boldmath{$m_{slag}$} \\
\midrule
1\_2 %MDPI:  % <-- Authors' reply: this is correct sample ID.
...
\bottomrule
\end{tabularx}
\footnotesize{%MDPI: Please confirm if the italics should be retained. % <-- Authors' reply: italics can be removed.
%MDPI: Please confirm if this sentence ''Notations in Table 2 and comments for abbreviations'' can be removed directly % <-- Authors' reply: ok, removed.
ID---sample number; %MDPI: We made table footer note into 1 paragraph and removed list item. Please confirm this revision. % <-- Authors' reply: Yes, we confirm.
 	 w---water ratio, \%; 
	 $\gamma$---density of soil ($\gamma_{dredged}$), $\frac{ton}{m^3}$; 
	 $m_t$, kg; 
	 $m_s$ = $\frac{m_t}{1+w}$; 
	 $m_w$ = $m_t - m_s$ (kg);
	 w/b is defined as a ratio of water/binder indicating the amount of binder;
...
The segments of lines are separated by measured strength values, appearing as fragments on the general trend lines, which depict the dynamics in relative deformation individually for various soil specimens. In contrast to the soil samples from test sites 1 to 8, the use of blended mixtures with different ratios of water/binder in cement/slag blends in tests 11--13 demonstrates more stable results of compression and strength gain. After achieving the peak compression, the deformation values stabilize at approximately 2\% of the values in soil samples compared to the original raw materials.} %MDPI: some changes made in this paragraph. %Authors' reply: yes, agree, correct.
...
\caption{P-wave %MDPI: Please check if an explanation should be added for dot line, and blue dots. % <-- Authors' reply: explanation is added in caption: "dotted line signifies the general trend for increase in the compressive strength; blue dots signify values of compressive strength for measured soil samples." 
 velocity as a function of the compressive strength. The dotted blue line signifies a general trend for the increase in the compressive strength of soil; blue dots signify values of the compressive strength for measured soil samples.\label{fig12}}
\end{figure}
...
Tests with iteratively changed binder parameters were used to select the best combinations by utilizing various w/b ratios in a recipe of blended binders. The differences in P-wave behavior and UCS indirectly indicate variations in the porosity of soil stabilized with various blended mixes (cement/slag) and w/b ratios, and these are compared with the soil samples recently stabilized, as shown in Figure \ref{fig12}. %MDPI: changes made in this paragraph. %Authors' reply: yes, agree.
...
\acknowledgments{The authors thank the reviewers for reading, suggestions and comments that improved an earlier version of this manuscript. %MDPI: please consider removing this part. Thank you very much. % <-- Authors' reply: ok, done (removed).
}
...
\begin{figure}[H]
\hspace{10pt}\makebox[0.90\linewidth][r]{%
\hspace{1pt}
	\begin{subfigure}[b]{.33\textwidth}
			\includegraphics[width=5cm]{Fig_K1.jpg}
		\captionsetup{position=bottom,justification=centering}\caption{}
	\end{subfigure} \hspace{3mm}
	\begin{subfigure}[b]{.33\textwidth}
			\includegraphics[width=5cm]{Fig_K2.jpg}
		\captionsetup{position=bottom,justification=centering}\caption{}
	\end{subfigure} \hspace{3mm}
	\begin{subfigure}[b]{.33\textwidth}
			\includegraphics[width=5cm]{Fig_K3.jpg}
		\captionsetup{position=bottom,justification=centering}\caption{}
	\end{subfigure} 
}\\
\vfill \vspace{1pt}
...
%\centering
		\vspace{3mm}
	\begin{subfigure}[b]{.33\textwidth}
			\includegraphics[width=5cm]{Fig_K4.jpg}
		\captionsetup{position=bottom,justification=centering}\caption{}
	\end{subfigure}\hspace{3mm}
	\begin{subfigure}[b]{.33\textwidth}
			\includegraphics[width=5cm]{Fig_K5.jpg}
		\captionsetup{position=bottom,justification=centering}\caption{}
	\end{subfigure}\hspace{3mm}
		\begin{subfigure}[b]{.33\textwidth}
			\includegraphics[width=5cm]{Fig_K6.jpg}
		\captionsetup{position=bottom,justification=centering}\caption{}
	\end{subfigure}
}\\
\vfill \vspace{6pt}
\caption{Grain %MDPI: 1. Please use periods as decimal sign instead of commas. e.g., "0,1" should be "0.1". 2. Please provide explanation for sub-figures (a-f). 3. Figure b, there is extra line at left side, please confirm if should be remove. 
% <-- Authors' reply: Figure A1 is corrected with required changes made: 1) periods are used as decimal sign instead of commas; 2) explanations are added; 3) . Figure b is checked and replotted.
 distribution in sediments showing dominated fine-grained silts (<0.063 mm): \textbf{(a)} soil samples with weight percent of grain content from 18 to 87\%; \textbf{(b)} soil samples with weight percent of grain content from 20 to 94\%; \textbf{(c)} soil samples with weight percent of grain content from 19 to 83\%; \textbf{(d)} soil samples with weight percent of grain content from 8 to 53\%; \textbf{(e)} soil samples with weight percent of grain content from 12 to 73\%; \textbf{(f)} soil samples with weight percent of grain content from 10 to 48\%.}\label{figA1}
\end{figure}
...
\newblock {\em Appl. Sci.} {\bf 2021}, {\em 11}, {1080.} %MDPI: Newly added information. Please confirm. %Authors' reply: yes, confirm.
...
\newblock
  Available online: \url{https://www.sis.se/en/produkter/construction-materials-and-building/construction-materials/cement-gypsum-lime-mortar/ssen197120112/} (accessed on 14 June 2023). %MDPI: Please add the access date (Format: Date Month Year). e.g., (accessed on 1 January 2020). %Authors' reply: yes, added access date.
...
\newblock {\em {Dyn. Geotech. Test. ASTM STP %MDPI: Please reconfirm journal name because different from information we found. % <-- Authors' reply: Yes, we confirm the "Dyn. Geotech. Test. ASTM STP".
}} {\bf 1978}, {\em
