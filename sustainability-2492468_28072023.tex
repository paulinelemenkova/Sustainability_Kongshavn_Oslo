...
The selection of binders used for stabilization of soil was adjusted according to our previous studies and several similar projects in Sweden. In mixing tests, a binder ratio of 30\% CEM I (SS-EN 197-1) \cite{SIS2011} and 70\% slag (SS-EN 15167-1) \cite{SIS2006} was selected as\hl{ }the most effective combination \hl{of binders }and homogenised with soil using \hl{the }KitchenAid eccentric mixer\hl{. After mixing, the soil samples were} placed in \hl{the }sampling sleeves, Figure \ref{fig06}. Before mixing binders into the dredged sediment masses, each specimen was homogenised for at least five minutes using a mixer. Normally, in large-scale projects, a Hobart mixer is used for large quantities of mortar or dredged soil material \cite{OLIVEIRA2019103110,RAMIREZ2023100920,Asi}. However, in this case, because the experiments were performed on a small-scale level, the volumes \hl{of soil }were too small to use a Hobart mixer. \hl{The mixing in the large scale can be performed using Hobart mixer which results in practically the same results since here the functionality of the device only consists on the volume of soil, but the mixing itself has the identical results. Therefore, the use of the smaller device was only adjusted to the small portion of the materials tested which is an acceptable practice in the SGI laboratory for small-scales experiments where parts of soil are tested.} Hence, the \hl{mixing} procedure was performed using the KitchenAid type eccentric mixer, see Figure \ref{fig06} (a). 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Results \textcolor{blue}{and Discussion}}
...
Figure \ref{fig08} shows a graph which illustrates\hl{ }variation \hl{in the values of} compressive strength with different proportions between cement and slag where the effects from binders are clearly visible. \hl{
Here the mechanical properties of soil were evaluated against the development of UCS in two ways: first, as a function of stabilisation by cement (as it is common in ordinary cement measurements, dark blue coloured), and second, as a function of stabilisation with slag (i.e., soil strength measured for a selected soil sample stabilised with slag, brown colour in Figure)}  \ref{fig08}. These tests were carried out on \hl{the }dredged masses \hl{collected }from \hl{the }Arendal 2 in Gothenburg. In the tests, the total amount of binder \hl{remains} the same \hl{while} the ratio between cement and slag \hl{varies} from 0\% slag to 90\% slag. Samples with 70\% slag give the highest compressive strength. \hl{Black} dots represent the results from the tests on \hl{the }Uniaxial Compressive Strength \hl{(UCS)}. Strength development in soil stabilised with different combinations of cement and slag varied \hl{was evaluated. The}  best \hl{combination of binders was detected to control their effects }on strength development \hl{which is} obtained at a slag admixture of 70\% of the total amount of binder, Figure \ref{fig08}. \hl{In this way,} the highest content of binders with the maximal achieved strength in soil samples \hl{can be evaluated}.
...
\hl{In the curves of Figure} \ref{fig09}, \hl{we noted almost linear correlation between the curing time and the increase in speed (i.e., sound velocity) and frequency of measured P-waves passing the soil samples, which indicates the improvement of the mechanical parameters of soil such as elastic (Young's) modulus, and indicating gain in soil strength over the period of curing. Moreover, one can conclude from the analysis of Figure} \ref{fig09} \hl{that for a selected binder blend (w/b 5, 6 and 7 for different test sites, see the legend in upper right corner of Figure} \ref{fig09}), \hl{stiffness and strength development (which corresponds to the E modulus) is approximately linearly related to the curing period, and is also well correlated with the values of P-wave, their sound velocity and frequency.}

\hl{Table} \ref{tab03} \hl{summarises the data on the measured samples and the results of the compressive strength (UCS, kPa) using mechanical tests with varied w/b ratio. More specifically, Table} \ref{tab03} shows the values of the compressive strength \hl{according to the different} w/b \hl{ratios }for specimens collected in various test sites (TS). \hl{The measurements of the UCS correspond to the columns 3, 6 and 9 in Table} \ref{tab03}). \hl{The} UCS tests were carried out according to the existing Swedish standard SS-EN 16907-4 \cite{SIS2018} with recommended deformation rate of 1 mm/min for soil specimens with a height of 100 mm. The results are summarised in Table \ref{tab03}. For the point $1\_2$, the w/b = 5, 6 and 7 were tested for each specimen. As the ratio between water and binder controls the strength, samples with w/b = 5 had higher strength compared to \hl{the }samples with w/b = 6 or 7, which proved the expected results. 
...
Table \ref{tab04} shows the compressive strength with relation to the effects of changed water/binder ratio (w/b) for the test points No. 5, 6 and 8, and additionally\hl{, }the P-wave velocity for test site 5. \hl{Here soil stabilised by the blended cement-slag mixtures and water added in various proportions into the mixture indicates well performance in stabilisation. This is achieved due to a balanced w/b proportions which outperforms that of soil treated by cement-only binders. }With regard to the obtained results of the UCS tests, P-wave measurements were performed to analyse \hl{the }correlation of the P-wave velocity \hl{against the} compressive strength in \hl{evaluated }soil samples. Therefore, after testing \hl{the }P-wave velocity, the values of the compressive strength were calculated based on the recorded data on speed of waves which correlate with the strength of the materials. These results are reported in Table \ref{tab04}. 
...
Figure \ref{fig10} shows a graph of correlation between the pressure (kPa) and relative deformation of the \hl{soil specimens collected} from \hl{the }sample tests site point $No. 1\_2$. The values of \hl{the }compressive strength were categorised as different lines recorded by \hl{the }measurements of specimens\hl{. Overall, the }values of \hl{the }compression not exceeding 900 kPa. Originally recorded by the UCS device, the data were modelled \hl{and evaluated statistically using the} Statistica software. \hl{The effectiveness of the UCS tests is ensured by the high precision measurements and force capacity in the MTS 810 testing machine which operates for various temperature and soil conditions. The stability precision ensures a wide total range of UCS values, since this MTS 810 machine measures soil strength at a high level of precision. The MTS machine was used for a complete period of testing i.e., over 60 days as in case of this study (2 months) which enabled to analyse the dynamics of gain in strength. }

The segments of lines are separated by measured strength values as fragments on the general trend lines which show the dynamics in relative deformation, identified individually for various soil specimens. \hl{In contrast to the soil samples from test sites 1 to 8, the use of blended mixtures with various ratios of water/binder in cement/slag blends in tests 11, 12 and 13 demonstrates more stable results of the compression and the gain in strength. After the achived peak in compression, the deformation values stabilise after approximately on 2\% of values in soil samples compared to original raw materials.}
...
The increase in relative deformation continues rapidly until \hl{the values of the }compression strength \hl{reach} 800 kPa after which it stabilises and then decreases. Possible distortions and noise on the samples 11 to 13 are identified and classified as unstable response to \hl{the content of }binder by \hl{the }selected \hl{soil }samples, since these \hl{specimens }were compressed with relatively low strength \hl{that} does not exceeds 300 kPa. Samples 1 to 8 demonstrate \hl{a} stable dynamics in measurements. In all cases, the relative deformation was measured until value of 2.2 \%. \hl{The comparison of the plots for soil samples collected in test sites (TS) from 1 to 13 suggests the direct effects from the water/binder ratio on the hydration of cement that leads to the gain in strength (UCS). Such findings are is necessary to account for the identification of best combinations of binders and w/b ratio while optimising binder blends and selecting the proportions of water in recipes.}

Figure \ref{fig11}\hl{ shows} a graph of \hl{the }pressure-deformation relationship for \hl{soil }samples collected from the test point No. 3 with three different levels of w/b \hl{which }are clearly visible in the graph. Randomly coloured line traces indicate selected sample specimens tested for \hl{evaluation of soil }deformation\hl{. }The first number in the numbering is the test point, in this case point $No. 1\_2$. The second number is a serial number. Samples 1 to 4 have w/b = 5, samples 6 to 8 have w/b = 6 and samples 11 to 13 have w/b = 7. \hl{The }variations in strength prove the expected results with the highest values for w/b = 5 and the lowest strength for w/b = 7\hl{, respectively}. 

\hl{As the curves in specimens 11 to 14 are almost identical with regard to the development of relative deformation and not increase 200 kPa in maximal values, this indicates that measurement of UCS tests will give a good prediction of the deformation-strength relationship and also extrapolated for the strength over the time exceeding the testing period. One can see in Figure }\ref{fig11} \hl{that the highest compression (over 800 kPa) is achieved by the soil collected in test sites 1 to 3. Furthermore, middle values corresponds to the soil collected in sites TS 6 to 9 of the testing samples, with maximal values below 330 kPa. The development of the compressive strength is related with the elastic modulus of soil. }

\begin{figure}[H]
\centering
	\includegraphics[width=13.0 cm]{Fig_11.jpg}
\caption{Correlation between \hl{the }compression (kPa) and relative deformation of 11 specimens from the sample test site No. 3. \label{fig11}}
\end{figure}

Accordingly, for \hl{the} sample point No. 3, the expected pattern is shown in the same way in \hl{soil }specimens with values of water/binder ratio between\hl{ }5, 6 and 7, see Figure \ref{fig11}. Here the fragments of the modelled lines were annotated and randomly coloured using \hl{the }IDs of soil samples and records on deformation value against \hl{the }compression (kPa). However, the specimens collected from the sample test point $3\_1$ exhibit \hl{a }slightly lower \hl{level of }strength compared to\hl{ }other two test samples with w/b 5. For samples from the point No. 3, less variation in stiffness was detected compared to the samples collected from the test point No. $1\_2$. \hl{Here} we also demonstrated the relationship between the compressive strength and \hl{a }relative deformation which reaches peak at 850 kPa, after which \hl{it }stabilises and \hl{then }decreases. 

%----------- subsection ----------->
\subsection{P-wave velocity vs UCS}

Figure \ref{fig12} shows a graph of the measured P-wave velocity as a function of the UCS. In this experiment, we \hl{evaluated} the ability of ultrasonic testing to leverage from \hl{the }P-wave velocities to strength gain in \hl{the }soil samples. The variations at a higher strength are completely normal. Since \hl{these} data are \hl{presented by the }independent soil samples, we first test P-wave responses passing through specimens of each soil sample tests based on their individual characteristics, such as water content, density and grain size, and evaluate the P-waves in soil samples stabilised with various w/b ratio at starting and ending time, and the selected intervals in between. \hl{The behaviour of strength over time in samples of soil stabilised by cement approximately resembles the logarithmic curve showing the relationship of hardening of the materials with increase in strength. }

Then we \hl{extracted the} information from \hl{the }measured records indicating \hl{the }P-wave speed changing over \hl{the }measured time, to evaluate the dynamics in strength gain for each \hl{soil }sample. The technical details of the P-wave values were recored in \hl{the }metadata including wave speed, maximal and mean values with relation to the UCS. The analysis of graph in Figure \ref{fig12} shows \hl{the }following data. The P-wave velocity \hl{shows} a stable increase from 415 m/s to 1170 m/s which is achieved by \hl{the }compressive strength of 1220 kPa. \hl{The speed of P-waves when passing the soil samples was monitored in a straightforward way by measuring the speed from sensors received by the ICP Accelerometer. The development of P-wave velocities continue after cementation of soil mixed with binders during stabilisation for a measured period, as can be seen in Figure} \ref{fig12}.

\hl{The parabola function well represents the relationship between P-wave velocity and compressive strength in Figure} \ref{fig12} \hl{which clearly depicts its nonlinear nature. The correlation of P-wave-velocity and compressive strength well differs it s from the curves presented in earlier plots and has a steeper curvature, since it represent the relationship of speed of the elastic waves on the stiffness  or porosity of the solid material. The tests were performed on soil samples which have varying w/b ratio in the binder mixture cement/slag content. Since the behaviour of elastic waves released by the accelerator and processed back by the received differs for samples stabilised with blended mixtures, various values for dots in Figure} \ref{fig12} \hl{well illustrates the effects and minor variations of w/b mixtures in slag-cement binders on soil strength. Nevertheless, the trend curve shows a general direction of the relationship between P-waves and strength in soil as a parabola function. Soil samples stabilised with cement/slag binders and different w/b ratios can be distinguished as individual dots on a graph.}
...
\hl{In a sense, long-term stabilised samples show a higher degree of strength and correspondent high P-waves while the ones recently stabilised have lower values and are comparable (e.g., for P-wave velocity of 440 or 600 m/s) and appear to have a similar behaviour of soil. In contrast, the highest value of P-wave speed (values over 100 m/s)was recorded by the highest UCS and long-term curing time with a higher dispersion of values. This is not surprising, since the balanced viscosity of water-binder ratio in cementitious binders and their rheological parameters are well known effective factors for soil strength, which is found by the indirect descriptors of strength that differ for individual samples. Using various w/b ratios in a recipe of blended binders, best combination were chosen using tests with iteratively changed binder parameters. The differences in P-waves behaviour and UCS indirectly indicate variations in the porosity of soil stabilised with various blended mixes (cement/slag) and w/b ratio, and compared them with the soil samples recently stabilised, Figure} \ref{fig12}. 

Figure \ref{fig13} shows a graph of the P-wave velocity in relation to the UCS for the Oslo project in comparison with selected Swedish projects on the stabilization of the dredged soil sediments. The results obtained in this survey are well in line with those obtained from the earlier Swedish projects. Thus, regarding the compressive strength and the relationship between the P-wave velocity and strength, the results of the tests in this study prove the expected estimations, Figure \ref{fig13}. \hl{The} results presented in this study demonstrate that \hl{the }stabilisation of dredged marine sediments with \hl{the }adjusted ratio of water-binder evaluated using sonic tests is an efficient and powerful framework which considers multiple variables affecting soil hardening and their attributes. 
...
\hl{In Figure} \ref{fig13}, \hl{the P-wave velocity has been evaluated by the samples collected from Arendal (A), V\"aster\aa s (V), Oslo (O) and Timr\aa (two sites T1 and T2) after the curing period. Here, the analysis of the UCS data interprets the degree of cement hydration indicating soil stabilization process from the speed of the P-waves that correlate to increased soil strength during stabilisation. }The maximal level of \hl{soil }strength is achieved at the w/b ratio of 5 where P-waves reach the recorded velocity of 1161 m/s. Such a good performance has a theoretical explanation of \hl{the }optimised plasticity and viscosity of binder recipe which improves the mechanical properties of \hl{the }cured soil. In particular, we derive the development of P-wave velocity with curing time\hl{ }up to 60 days to demonstrate the effects from \hl{the }two factors: the dynamics of strength over time and the influence of water-binder percentage on soil setting. 

\hl{This part of the works was performed as a critical test case to comparatively evaluate the behaviour of the treated soil samples relative to the locations and environmental conditions of the original raw materials. The regression demonstrates the statistical processing of the data collected in Oslo (black crosses) at the end of the curing period for different combinations of w/b ratios. We visually analysed the similarity between the curves for these soil samples tested with varied w/b proportions and locations to compare the effects of the increase in ratio of stabilizing agents while decreasing water content. As a result, the graph represents a clearly visual similarity in curves with soil collected from Timr\aa site having steep curves comparing to others, and representing soil samples with the highest percentage of binders with regard to water. }
...
From a pure civil engineering perspective, it seems more reasonable to model each soil sample as the superposition of factors that include variations in binder proportions and water content \cite{Bumanis,HAN2022e01660,HAY2023131529}. Shared across all \hl{the soil }samples of the same\hl{ }probe, such experiments enable to evaluate the effects of \hl{the }water-binder dosage on stabilisation process and environmental properties\hl{. Thus, such approach enables to evaluate }leaching of heavy metals and toxic substances\hl{, }with regard to the individual properties of soil: density, moisture, grain size, type, texture, content of clay/sand, amount of organic matter. In this paper, we demonstrated that this representation allow us to account for the differences between \hl{various }dosage in water-binder mixtures and their effects on stabilisation in the same class of soil samples. Furthermore, we showed the non-linearity in the soil modelling process when evaluated the relationship between \hl{the }soil texture and water-binder percentage. 

Our results are relevant for \hl{the }analysis of geotechnical properties of soil since they provide new information about the strength parameters of soil stabilised with various water-binder mixtures. Adjusted proportions of binders allow us to focus on soil hardening through the stabilisation workflow with optimised water-binder ratio. We \hl{furthermore }demonstrated \hl{that} the water-binder ratio is \hl{an }important factor in s/s \hl{soil }treatment techniques aimed at improving \hl{the initially }poor engineering properties of dredged marine sediments prior to reuse \hl{as construction materials}. \hl{Thus, the measurements of UCS investigated here provide a concrete illustration of the role of w/b recipes in hardening of clayey soil (dredged sediments collected from the harbour of Oslo) stabilised with cement/slag in various ratio. Since the ratio of these binders remained constant, and we only changes water content, this enabled to see the effects of stiff/wet binder on hydration of cement and thus soil stabilization with various proportions of water/binder.} 

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Conclusions}

In this paper, we developed an effective framework for modelling \hl{the }w/b effects on stabilisation of clayey dredged materials collected from the harbour of Oslo, Norway. We proposed two methods for evaluating soil strength stabilised using different combination of water/binder ratio, namely, using UCS tests implemented by the MTS 810 servohydraulic universal testing machine, and using sonic tests with evaluated velocities of pulsed P-waves. Inspired by the existing approaches of seismic methods applied to civil engineering problems \cite{app13042745,HARRYPERSADDANIEL2022104557}, we prove that a combination of \hl{the }state-of-the-art methods with non-destructive seismic tests can model dependency of the soil strength from binder proportions, w/b ratio and curing time. \hl{Specifically, the analysis of the presented data shows that the lowest P-wave seismic velocity of 1200 m/s is demonstrated by the highest strength and corresponding UCS values, which is achieved by the optimal proportion of water with the lowest content of binder, i.e., w/b=7.99, and the lowest absolute content of binder such as 2.022 mB/g. This shows the performance of physical mechanisms in soil particles operating during the hydration of cement in the process of soil stabilisation, and setting of soil-binder mixtures, which was evaluated by seismic tests. }

Under the framework of the presented modelling of stabilised soil properties, it was shown that various w/b dosage provides essential information at the variable feature level for modelling \hl{the }dependency of soil strength on \hl{a }binder content. Besides, \hl{the }improved binder properties enhance key characteristics of \hl{the }construction materials, such as compressive, tensile and shear strength, ductility and flexure \cite{Reddy2022,Maheretal2008,MaherHo1994}. Finally, the optimal combination of \hl{the }w/b ratio are obtained for dredged clayey sediments in \hl{a} laboratory scale which can be generalised at the \hl{large-scale }project level. \hl{Hence, as it can be noticed in Table} \ref{tab04} \hl{summarising the results of the compressive strength in various w/b proportions of binders used for soil stabilisation, seismic (P-waves velocities) and mechanical measurements show an agreement in significant increase in strength of soil samples over period of curing with increased binder content and w/b=5 when the strength measured by UCS tests overstepped 500 kPa and P-waves were higher than 1000 m/s. This proved positive effects from the water/binder ratio of 5 on clayey soil stabilisation. }

The proposed modelling of soil properties at a laboratory scale minimises the costs of \hl{the }project-run works where large quantities of sediments must be dredged repeatedly for the needs of marine transportation. \hl{In view of the above said, }the contributions of the this paper are threefold:
...
\begin{itemize}
  ...
  \item The assumptions on the effects from \hl{the }changed w/b dosage on\hl{ }behaviour \hl{of the marine sediments }is tested by adding various percentage of admixtures of cement/slag as binders.
  ...
  \item We showed that the combination of \hl{the }P-waves and UCS methods for evaluating soil properties outperforms the standard state-of-the-art approach in two ways. 
  	\begin{itemize}
		...
		\item Second, the analysed sensitivity of the ICP Accelerometer for measured resonance frequencies of sediments proved its high perception to varied \hl{level of }stiffness and viscosity of \hl{soil }samples stabilised with different w/b dosage.
	\end{itemize}
\end{itemize}

Practical essential advantage of the presented work consists in the optimisation of \hl{the }soil stabilisation process in marine harbours \hl{prior to construction works where large quantities of soil are to be stabilised. For instance, optimisation of the processing of }millions of tons of the dredged sediment material \hl{using} s/s techniques \hl{increases cost-effectiveness}. Thus, the optimisation of s/s techniques in such large-scale projects is crucial. The optimisation of \hl{the }w/b dosage supports finding of the best analytical solution for cost-effective s/s treatment of soil from marine harbours, when sediments are collected regularly and in large quantities. Using our data, the information on soil strength depending on \hl{the }w/b ratio and deformation of clayey soil specimens in process of compression can be generalised in large-scale projects. \hl{Overall,  the results of seismic and UCS measurements demonstrated in this study show a similar trend and accordance in the estimated strength in stabilised specimens which increased over time of curing in stabilised soil samples.}
...
\acknowledgments{The authors thank the reviewers for reading, suggestions and comments that improved an earlier version of this manuscript. The authors express sincere gratitude to the Editorial Office of Sustainability, MDPI for financial support of this publication.}
